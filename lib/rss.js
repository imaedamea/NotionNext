...
export async function generateRss(props) {
  ...
  // 插入 <follow_challenge> 标签
  const followChallenge = `
    <follow_challenge>
      <feedId>62150511499401216</feedId>
      <userId>61967744571885568</userId>
    </follow_challenge>
  `
  rssContent = rssContent.replace('</channel>', `${followChallenge}</channel>`)

  try {
    fs.mkdirSync('./public/rss', { recursive: true })
    fs.writeFileSync('./public/rss/feed.xml', rssContent)
